...
BOOT_CODE void clock_sync_test(void)
{
    ...
    /*
     * clock_sync_test does not consistently work on QEMU, try and detect if we
     * are running as a guest and skip the test.
     */
    ...
    if ((ebx == CPUID_KVM_EBX && ecx == CPUID_KVM_ECX && edx == CPUID_KVM_EDX)
        ...
        printf("clock_sync_test[%d]: disabled, QEMU was detected\n", (int)getCurrentCPUIndex());
        return;
    }
#endif
    ticks_t t, t0;
