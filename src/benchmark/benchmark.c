...
exception_t handle_SysBenchmarkResetLog(void)
{
    ...
    NODE_STATE(benchmark_kernel_instructions) = 0;
    NODE_STATE(ksCurThread)->benchmark.schedule_start_instruction = ksEnterInst;
...
exception_t handle_SysBenchmarkDumpAllThreadsUtilisation(void)
{
    ...
    printf("  \"BENCHMARK_TOTAL_KERNEL_INSTRUCTIONS\":%lu,\n", (word_t) NODE_STATE(benchmark_kernel_instructions));
