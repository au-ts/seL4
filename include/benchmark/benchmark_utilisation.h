...
static inline void benchmark_utilisation_switch(tcb_t *heir, tcb_t *next)
{
    ...
    if (likely(NODE_STATE(benchmark_log_utilisation_enabled))) {
        ...
        if (likely(ARCH_NODE_STATE(ksEnter) > heir->benchmark.schedule_start_time)) {

            heir->benchmark.utilisation += (ARCH_NODE_STATE(ksEnter) - heir->benchmark.schedule_start_time);

        } else {
#ifdef CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT
            heir->benchmark.utilisation += (UINT32_MAX - heir->benchmark.schedule_start_time) + ARCH_NODE_STATE(ksEnter);
            armv_handleOverflowIRQ();
#endif /* CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT */
        }

        /* Reset next thread utilisation */
        next->benchmark.schedule_start_time = ARCH_NODE_STATE(ksEnter);
...
static inline void benchmark_utilisation_finalise(void)
{
    ...
    NODE_STATE(benchmark_end_time) = ARCH_NODE_STATE(ksEnter);
